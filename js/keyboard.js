...
function VKI_buildKeyboardInputs() {
  ...
  for (var x = 0, inputCount = 0, elem; elem = inputElems[x++];) {
    if (elem) {
      for (var y = 0, keyid = "", ex; ex = elem[y++];) {
        if ((ex.nodeName == "TEXTAREA" || ex.type == "text" || ex.type == "password") && ex.className.indexOf("search_box") > -1) {
          if (!ex.id) {
            do { keyid = 'keyboardInputInitiator' + inputCount++; } while (document.getElementById(keyid));
            ex.id = keyid;
          } else keyid = ex.id;
          var keybut = document.createElement('img');
              keybut.src = ''+base_url+'/js/keyboard.png';
              keybut.alt = "الكيبورد كربي";
              keybut.className = "keyboardInputInitiator";
              keybut.title = "إضغط هنا لإظهار الكيبورد";
 ...
 var logo = document.createElement('span');
            ...
            logo.appendChild(document.createTextNode("كيبورد عربي"));
		...
		//logo.onclick = function () { location.href='http://www.arabic-keyboard.org';};
         logo.onclick = function ()  {window.open('http://www.arabic-keyboard.org');};
          td.appendChild(logo);


        var clearer = document.createElement('span');
            clearer.id = "keyboardInputClear";
            clearer.appendChild(document.createTextNode("تفريغ ما كتبت"));
            clearer.title = "إفراغ النمودج لإعادة الكتابة";
